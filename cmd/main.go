...
func init() {
	...
	utilruntime.Must(nacosiov1.AddToScheme(scheme))
...
func main() {
	...
	flag.StringVar(&metricsAddr, "metrics-bind-address", ":7847", "The address the metric endpoint binds to.")
	flag.StringVar(&probeAddr, "health-probe-bind-address", ":7846", "The address the probe endpoint binds to.")
	flag.BoolVar(&enableLeaderElection, "leader-elect", false,
		"Enable leader election for controller manager. "+
			"Enabling this will ensure there is only one active controller manager.")
	flag.BoolVar(&enableWebhook, "enable-webhook", false, "Enable webhook for validation and defaulting")
	opts := zap.Options{
		Development: true,
	}
